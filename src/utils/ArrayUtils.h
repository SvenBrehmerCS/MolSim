/**
 * @file
 * @brief Implements utility functions and operators for iterable data containers
 * @author F. Gratl
 * @date 12/13/19
 */
...
namespace ArrayUtils {
    ...
    [[nodiscard]] std::string to_string(
        const Container& container, const std::string& delimiter = ", ", const std::array<std::string, 2>& surround = { "[", "]" }) {
     ...
     */
    template <class Container, class F> inline Container elementWisePairOp(const Container& lhs, const Container& rhs, F binaryFunction) {
        Container ret = lhs;
 ...
 */
template <class Container> std::enable_if_t<ArrayUtils::is_container<Container>::value, bool> operator==(const Container& lhs, const Container& rhs) {
