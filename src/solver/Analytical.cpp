...
static constexpr double error_margine = 0.001;

bool solver::is_ballanced_dance_solution(const Particle& p1, const Particle& p2, const double t) {
    ...
    if (ArrayUtils::L2Norm(p1.getX() - x1) < error_margine && ArrayUtils::L2Norm(p2.getX() - x2) < error_margine) {
        return true;
    }
...
bool solver::is_center_dance_solution(const Particle& p1, const Particle& p2, const Particle& center, const double t) {
    ...
    if (ArrayUtils::L2Norm(p1.getX() - x1) < error_margine && ArrayUtils::L2Norm(p2.getX() - x2) < error_margine
        && ArrayUtils::L2Norm(center.getX() - x_center) < error_margine) {
        return true;
    }
...
bool solver::is_evasion_solution(const Particle& p, const double t) {
    ...
    if (ArrayUtils::L2Norm(p.getX() - x) < error_margine) {
        return true;
    }
