...
namespace physicsCalculator {
    LJSmoothCalculator::LJSmoothCalculator(const Environment& new_env, const std::shared_ptr<ParticleContainer>& new_cont, const double rl)
        : Calculator { new_env, new_cont }
        , r_l_squ { rl * rl }
        , r_l { rl }
        , r_c_squ { env.get_r_cutoff() * env.get_r_cutoff() } {
        // Initialize the forces
        calculateF();
    }

    LJSmoothCalculator::LJSmoothCalculator(const Environment& new_env, const std::vector<Particle>& particles, const double rl,
